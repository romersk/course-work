...
public class DtpSystem {
    ...
    public void toCalculate(ActionEvent actionEvent) throws IOException, ClassNotFoundException {
        ...
        double size = (double) Math.round((double) fieldDOne * fieldDTwo * (1 + fieldDThree/100) * 100) / 100;
        Salary salary = new Salary();
        salary.setMethod("Повременно-премиальная");
        salary.setSize(size);
    ...
    public void toSaveFile(ActionEvent actionEvent) {
        if (!result.getText().equals("")) {
            ...
            if (dir != null) {
                Data data = Data.getInstance();
                String way = dir.getAbsolutePath() + "/" + data.getUser().getLogin() + "-DTP.txt";
                try(FileWriter writer = new FileWriter(way, false))
                {
                    writer.write("Часовая тарифная ставка равна " + String.valueOf(fieldDOne) + "\n");
                    writer.write("Количество часов, отработанных работником, равно " + String.valueOf(fieldDTwo) + "\n");
                    writer.write("Процент премиальных доплат " + fieldThree.getText() + "\n");
